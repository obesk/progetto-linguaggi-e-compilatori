...
/* Il codice seguente sulle prime non è semplice da comprendere.
   ...
   La chiamata di questa utility restituisce un'istruzione IR che alloca un
   double in memoria e ne memorizza il puntatore in un registro SSA cui viene
   attribuito il nome passato come secondo parametro. L'istruzione verrà scritta
   all'inizio dell'entry block della funzione passata come primo parametro. Si
   ricordi che le istruzioni sono generate da un builder. Per non interferire
   con il builder globale, la generazione viene dunque effettuata con un builder
   temporaneo TmpB
*/
...
// metodo omonimo presente nel nodo root (il puntatore root è stato scritto dal
// parser)
void driver::codegen() { root->codegen(*this); };
...
// registro SSA il puntatore alla memoria allocata. Generare il codice
// corrispondente ad una varibile equivale dunque a recuperare il tipo della
// variabile allocata e il nome del registro e generare una corrispondente
// istruzione di load Negli argomenti della CreateLoad ritroviamo quindi: (1) il
// tipo allocato, (2) il registro SSA in cui è stato messo il puntatore alla
// memoria allocata (si ricordi che A è l'istruzione ma è anche il registro,
// vista la corrispodenza 1-1 fra le  due nozioni), (3)
...
// Vengono ricorsivamente generati il codice per il primo e quello per il
// secondo operando. Con i valori memorizzati in altrettanti registri SSA si
...
Value *CallExprAST::codegen(driver &drv) {
  ...
  // Se la funzione non viene trovata (e dunque non è stata precedentemente
  // definita) viene generato un errore
  ...
  // del builder, che viene chiamato subito dopo per la generazione
  // dell'istruzione IR di chiamata
...
Value *IfExprAST::codegen(driver &drv) {
  ...
  // previste nel "ramo" true del condizionale potrebbe dare luogo alla
  // creazione di altri blocchi, che naturalmente andrebbero inseriti prima di
  // FalseBB
  ...
  // 1) Dapprima si crea il nodo PHI specificando quanti sono i possibili nodi
  // sorgente 2) Per ogni possibile nodo sorgente, viene poi inserita
  // l'etichetta e il registro
...
Value *BlockExprAST::codegen(driver &drv) {
  // Un blocco è un'espressione preceduta dalla definizione di una o più
  // variabili locali. Le definizioni sono opzionali e tuttavia necessarie
  // perché l'uso di un blocco abbia senso. Ad ogni variabile deve essere
  // associato il valore di una costante o il valore di un'espressione.
  // Nell'espressione, arbitraria, possono chiaramente comparire simboli di
  // variabile. Al riguardo, la gestione dello scope (ovvero delle regole di
  // visibilità) è implementata nel modo seguente, in cui, come esempio,
  // consideremo la definzione: var y = x+1 1) Viene dapprima generato il codice
  // per valutare l'espressione x+1.
  //    L'area di memoria da cui "prelevare" il valore di x è scritta in un
  //    registro SSA che è parte della (rappresentazione interna della)
  //    istruzione alloca usata per allocare la memoria corrispondente e che è
  //    registrata nella symbol table Per i parametri della funzione,
  //    l'istruzione di allocazione viene generata (come già sappiamo) dalla
  //    chiamata di codegen in FunctionAST. Per le variabili locali viene
  //    generata nel presente contesto. Si noti, di passaggio, che tutte le
  //    istruzioni di allocazione verranno poi emesse nell'entry block, in
  //    ordine cronologico rovesciato (rispetto alla generazione). Questo perché
  //    la routine di utilità (CreateEntryBlockAlloca) genera sempre all'inizio
  //    del blocco.
  // 2) Ritornando all'esempio, bisogna ora gestire l'assegnamento ad y
  // gestendone la visibilità.
  //    Come prima cosa viene generata l'istruzione alloca per y.
  //    Questa deve essere inserita nella symbol table per futuri riferimenti ad
  //    y all'interno del blocco. Tuttavia, se un'istruzione alloca per y fosse
  //    già presente nella symbol table (nel caso y sia un parametro)
  //    bisognerebbe "rimuoverla" temporaneamente e re-inserirla all'uscita del
  //    blocco. Questo è ciò che viene fatto dal presente codice, che utilizza
  //    al riguardo il vettore di appoggio "AllocaTmp" (che naturalmente è un
  //    vettore di di (puntatori ad) istruzioni di allocazione
  ...
  for (int i = 0, e = Def.size(); i < e; i++) {
    ...
    // (in questo caso) non restituisce un registro SSA ma l'istruzione di
    // allocazione
  ...
  // Il valore del costrutto/espressione var è ovviamente il valore (il registro
  // SSA) restituito dal codice di valutazione dell'espressione
  return blockvalue;
};
...
const std::string &VarBindingAST::getName() const { return Name; };
...
AllocaInst *VarBindingAST::codegen(driver &drv) {
  ...
  // L'istruzione di allocazione (che include il registro "puntatore" all'area
  // di memoria allocata) viene restituita per essere inserita nella symbol
  // table
  return Alloca;
};
...
PrototypeAST::PrototypeAST(std::string Name, std::vector<std::string> Args)
    : Name(Name), Args(std::move(Args)),
      emitcode(true){}; // Di regola il codice viene emesso
...
void PrototypeAST::noemit() { emitcode = false; };

Function *PrototypeAST::codegen(driver &drv) {
  // Costruisce una struttura, qui chiamata FT, che rappresenta il "tipo" di
  // una funzione. Con ciò si intende a sua volta una coppia composta dal
  // tipo del risultato (valore di ritorno) e da un vettore che contiene il
  // tipo di tutti i parametri. Si ricordi, tuttavia, che nel nostro caso
  // l'unico tipo è double.

  // Prima definiamo il vettore (qui chiamato Doubles) con il tipo degli
  // argomenti
  std::vector<Type *> Doubles(Args.size(), Type::getDoubleTy(*context));
  // Quindi definiamo il tipo (FT) della funzione
  FunctionType *FT =
      FunctionType::get(Type::getDoubleTy(*context), Doubles, false);
  // Infine definiamo una funzione (al momento senza body) del tipo creato e con
  // il nome presente nel nodo AST. ExternalLinkage vuol dire che la funzione
  // può avere visibilità anche al di fuori del modulo
  Function *F = Function::Create(FT, Function::ExternalLinkage, Name, *module);

  // Ad ogni parametro della funzione F (che, è bene ricordare, è la
  // rappresentazione llvm di una funzione, non è una funzione C++) attribuiamo
  // ora il nome specificato dal programmatore e presente nel nodo AST relativo
  // al prototipo
  unsigned Idx = 0;
  for (auto &Arg : F->args())
...
Function *FunctionAST::codegen(driver &drv) {
  ...
  // Si noti che il builder conosce il registro che contiene il puntatore
  // all'area perché esso è parte della rappresentazione C++ dell'istruzione di
  // allocazione (variabile Alloca)
  ...
  for (auto &Arg : function->args()) {
    ...
    // Registra gli argomenti nella symbol table per eventuale riferimento
    // futuro
...
const std::string &GlobalVariableAST::getName() const { return Name; };

Value *GlobalVariableAST::codegen(driver &drv) {
  Constant *initialValue = ConstantFP::get(Type::getDoubleTy(*context), 0.0);

  GlobalVariable *GV =
      new GlobalVariable(*module, Type::getDoubleTy(*context), false,
                         GlobalValue::ExternalLinkage, initialValue, Name);
  if (!GV)
    return nullptr;
  GV->print(errs());
  fprintf(stderr, "\n");
